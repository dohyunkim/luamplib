...
do
  ...
  local mplibcolorfmt = {
    ...
    l3color = is_defined"__color_select:nn" and tableconcat{
      ...
      or "\\def\\mplib@tempb#1#2{\\str_case:nnF{#1}\z
       {{rgb}{\\mplibtmptoks{pdf:bc [#2]}}\z
        {cmyk}{\\mplibtmptoks{pdf:bc [#2]}}\z
        {gray}{\\mplibtmptoks{pdf:bc [#2]}}}\z
       {\\prop_get:NnN\\g__color_colorants_prop{#1}\\l_tmpa_tl\z
        \\tl_if_eq:NNTF\\l_tmpa_tl\\q_no_value\z
         {\\No_Support_of_DeviceN_in_DVI_luamplib\\mplibtmptoks{ERROR}}\z
         {\\prop_get:NeN\\g__color_backend_colorant_prop{/\\l_tmpa_tl}\\l_tmpa_tl\z
          \\mplibtmptoks\\expanded{{pdf:bc \\l_tmpa_tl\\space [#2]}}}}}\z
      \\color_export:nnN%s{backend}\\l_tmpa_tl\\mplibtmptoks\\expandafter{\\l_tmpa_tl}\z
      \\expandafter\\mplib@tempb\\l_tmpa_tl" ,
  }
...
luamplib.shadecolor = function (str)
  ...
  if res:find" cs " or res:find"@pdf.obj" then -- spot color shade
    ...
    for _,v in ipairs( str:explode"!" ) do
      if not v:find("^%s*%d+%s*$") then
        if not is_defined(format("l__color_named_%s_prop", v)) then
          l3 = false; break
        end
      end
    end
